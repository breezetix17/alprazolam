...
static const char* indicators[13] = { "eb", "jb", "lj", "mj", "ps", "ej", "lb", "ad", "fm", "air", "em", "ast", "bast" };
...
static const char* EdgebugTypes[] = { "alprazolam","lobotomy" };
    ...
    ImGui::NextColumn(); {
        ImGui::BeginChild(("legitbot.settings"), ImVec2{ }, true, ImGuiWindowFlags_MenuBar); {
            ...
            if (c::aimbot::aimbot) {
                ...
                ImGui::SliderInt(("##aimfov"), &settings->fov, 0, 180);
                ImGui::Checkbox("silent aim", &settings->silent);
                if (!settings->silent) {
                    ImGui::Text(("smoothing"));
                    ImGui::SliderInt(("##aimsmooth"), &settings->smooth, 0, 100);
                }
                ImGui::Checkbox("enable rcs", &settings->rcs);
                if (settings->rcs) {
                    ImGui::SliderInt(("##aimrcs"), &settings->rcs_p, 0, 100);
                }
                ImGui::Checkbox("enable autowall", &settings->autowall_b);
                if (settings->autowall_b) {
                    if (!settings->autowall_lethal) {
                        ImGui::Text(("autowall min damage"));
                        ImGui::SliderInt(("##awmin"), &settings->autowall_dmg, 1, 100);
                    }
                    ImGui::Checkbox("autowall if lethal", &settings->autowall_lethal);
                }
            }
        ...
        ImGui::BeginChild(("visuals.other"), ImVec2{ }, true, ImGuiWindowFlags_MenuBar); {
            ...
            if (c::visuals::shadows) {
                ...
                if (c::visuals::dynamic_shadows) {
                    ImGui::Text(("shadow rotation speed"));
    ...
    ImGui::Columns(2, nullptr, false); {
        ImGui::BeginChild(("misc.movement"), ImVec2(0, 190), true, ImGuiWindowFlags_MenuBar); {
            ...
            if (c::movement::mini_jump) {
                ...
                ImGui::Checkbox(("force edgejump##2"), &c::movement::mini_jump_ej);
            }
            ImGui::Checkbox(("edgejump on ladder"), &c::movement::edge_jump_on_ladder);
            if (c::movement::edge_jump_on_ladder) {
                ImGui::Keybind(("laj key"), &c::movement::ladder_jump_key, &c::movement::ladder_jump_key_s);
            }
            ...
            if (c::movement::edge_bug) {
                ...
                ImGui::Checkbox(("ultra edgebug"), &c::movement::edge_bug_strafe);
                if (c::movement::edge_bug_strafe) {
                    ...
                    if (c::movement::edge_bug_angle_limit > 0) {
                        ...
                        ImGui::Checkbox(("silent edgebug"), &c::movement::edge_bug_silent);
                    }
                }
            ...
            ImGui::Checkbox(("auto ladderglide"), &c::movement::ladder_bug);
            ...
            ImGui::Checkbox(("epstein mode"), &c::movement::auto_strafe);
            if (c::movement::auto_strafe) {
                ImGui::Keybind(("epstein mode key"), &c::movement::auto_strafe_key, &c::movement::auto_strafe_key_s);
            }
            ...
            if (c::assist::assist)
            {
                ...
                if (c::assist::pixelsurf_assist)
                {
                    ...
                    if (c::assist::assist_broke_hop) {
                        ...
                        ImGui::SliderFloat(("##staminamax"), &c::assist::assist_stamina_value, 0.f, 80.f, ("%.1f%"));
                    }
    ...
    ImGui::NextColumn(); {
        ImGui::BeginChild(("misc.config"), ImVec2(0, 190), true, ImGuiWindowFlags_MenuBar); {
            ...
            ImGui::NextColumn(); {
                ...
                if (ImGui::InputTextWithHint("##config.file", "create new...", buffer, sizeof(buffer), ImGuiInputTextFlags_EnterReturnsTrue)) {
                    ...
                    //std::stringstream ss;
                   // ss << "\x08" << " \x08" << "%c" << "alprazolam" << "\x08" << "\x08 | created config " << (std::string)buffer;

                    if (interfaces::engine->is_in_game()) {
                        interfaces::chat_element->chatprintf("#alprazolam#_print_created");
                    }
                ...
                if (ImGui::BeginPopup(("config save popup"))) {
                    ImGui::Text(("are you sure you want to save this config? "));
                    ...
                    for (auto i = 0; i < IM_ARRAYSIZE(choices1); i++)
                        if (ImGui::Selectable(choices1[i]))
                            if (i == 0) {
                                if (config_index == -1) {
                                    if (interfaces::engine->is_in_game()) {
                                        interfaces::chat_element->chatprintf("#alprazolam#_print_failed");
                                    }
                                }
                                else {
                                    c::save(config_index);
                                    //std::stringstream ss;
                                    //ss << "\x08" << " \x08" << "%c" << "alprazolam" << "\x08" << "\x08 | saved config " << c::configs.at(config_index);

                                    if (interfaces::engine->is_in_game()) {
                                        interfaces::chat_element->chatprintf("#alprazolam#_print_saved");
                                    }
                                }
                            }
                ...
                if (ImGui::Button("load", ImVec2(-1, 15))) {
                    if (config_index == -1) {
                        ...
                        c::load(config_index);
                        //std::stringstream ss;
                        //ss << "\x08" << " \x08" << "%c" << "alprazolam" << "\x08" << "\x08 | loaded config " << c::configs.at(config_index);

                        if (interfaces::engine->is_in_game()) {
                            interfaces::chat_element->chatprintf("#alprazolam#_print_loaded");
                ...
                if (ImGui::BeginPopup(("load only popup"))) {
                    ImGui::Text(("what settings do you want to load? "));
                    ...
                    for (auto i = 0; i < IM_ARRAYSIZE(choices2); i++)
                        if (ImGui::Selectable(choices2[i]))
                            ...
                            if (config_index == -1) {
                                ...
                                if (interfaces::engine->is_in_game()) {
                                    interfaces::chat_element->chatprintf("#alprazolam#_print_loaded_aimbot");
                                }
                            }
                            else if (i == 1) {
                                ...
                                if (interfaces::engine->is_in_game()) {
                                    interfaces::chat_element->chatprintf("#alprazolam#_print_loaded_visuals");
                                }
                                menu::load_font_index();
                                surface::initialize();
                                menu::fonts_initialized = true;
                                ...
                                if (interfaces::engine->is_in_game()) {
                                    interfaces::chat_element->chatprintf("#alprazolam#_print_loaded_mvm");
                                }
                            }
                            else if (i == 3) {
                                ...
                                if (interfaces::engine->is_in_game()) {
                                    interfaces::chat_element->chatprintf("#alprazolam#_print_loaded_indicators");
                                }
                            }
                            else if (i == 4) {
                                c::load_skins(config_index);
                                if (interfaces::engine->is_in_game()) {
                                    interfaces::chat_element->chatprintf("#alprazolam#_print_loaded_skins");
                                    features::skins::forcing_update = true;
                                }
                            }
                            else if (i == 5) {
                                ...
                                if (interfaces::engine->is_in_game()) {
                                    interfaces::chat_element->chatprintf("#alprazolam#_print_loaded_misc");
                                }
                            }
                ...
                if (ImGui::Button("refresh", ImVec2(-1, 15))) {
                    ...
                    if (interfaces::engine->is_in_game()) {
                        interfaces::chat_element->chatprintf("#alprazolam#_print_refreshed");
                    }
                }
                ImGui::PopItemWidth();
                ImGui::PopStyleVar();
            ...
            if (ImGui::Button(("build info"), ImVec2(-1, 15))) {
                ImGui::OpenPopup(("info popup"));
            }
            if (ImGui::BeginPopup(("info popup"))) {
                ImGui::TextColored(ImVec4(menu::menu_accent[0], menu::menu_accent[1], menu::menu_accent[2], 1.f), "spelling fixed by table (devs are dyslexic)");
        ...
        ImGui::BeginChild(("misc.main"), ImVec2{ }, true, ImGuiWindowFlags_MenuBar); {
            ...
            if (c::misc::show_spotify_currently_playing) {
                ...
                ImGui::Combo(("##player_type"), &c::misc::player_type, "alprazolam\0lobotomy");
            ...
            if (c::misc::spectators_list) {
                ...
                if (c::misc::spectatorlist_type == 0) {
                    ...
                    ImGui::SliderInt("spectator list width", &c::misc::spectatorlist_w, 195, 500);
                }
            }
            ...
            if (ImGui::Button("unlock hidden convars")) {
    ...
    ImGui::Columns(2, nullptr, false); {
        ImGui::BeginChild(("fonts.main"), ImVec2{ }, true, ImGuiWindowFlags_MenuBar); {
            ...
            if (ImGui::Button("reload fonts")) {
                ...
                if (interfaces::engine->is_in_game()) {
                    interfaces::chat_element->chatprintf("#alprazolam#_print_reloaded");
                }
                menu::fonts_initialized = true;
                menu::load_font_index();
                surface::initialize();
    ...
    ImGui::Columns(2, nullptr, false); {
        ImGui::BeginChild(("skins.item"), ImVec2{0, 190 }, true, ImGuiWindowFlags_MenuBar); {
            ...
            case 0:
                ...
                ImGui::Combo(("##knifemodel"), &c::skins::knife_changer_model, skinchanger_knives, IM_ARRAYSIZE(skinchanger_knives));
                break;
            case 1:
                ImGui::Checkbox(("enabled##gloves"), &c::skins::gloves_endable);
                ImGui::Combo("##glovesmodel", &c::skins::gloves_model, skinchanger_gloves, IM_ARRAYSIZE(skinchanger_gloves));
                break;
            case 2:
                ImGui::Checkbox(("enabled##weapon"), &c::skins::weapon_endable);
                ImGui::Combo("##weaponmodel", &c::skins::weapon_model, skinchanger_weapons, IM_ARRAYSIZE(skinchanger_weapons));
                break;
            }
        ...
        ImGui::BeginChild(("skins.skins"), ImVec2{ }, true, ImGuiWindowFlags_MenuBar); {
            ...
            case 2:
                ...
                ImGui::ListBoxHeader("##weaponskin", ImVec2(-1, 200)); {
                    ...
                    for (int i = 0; i < features::skins::parser_skins.size(); i++) {
                        bool is_selected = (settings->vector_paint_kit == i);
                        ...
                        if (filter.PassFilter(name.c_str())) {
                            ...
                            if (ImGui::Selectable(features::skins::parser_skins[i].name.c_str(), &is_selected)) {
                                settings->vector_paint_kit = i;
                                settings->paint_kit_index = features::skins::parser_skins[settings->vector_paint_kit].id;
                            }
                ...
                if (settings->wpn_skin_custom_clr) {
                    ImGui::SameLine(group_w - 20);
                    ImGui::ColorEdit4(("##modulate1"), settings->wpn_skin_modulation1, no_alpha);
                    ImGui::SameLine(group_w - 40);
                    ImGui::ColorEdit4(("##modulate2"), settings->wpn_skin_modulation2, no_alpha);
                    ImGui::SameLine(group_w - 60);
                    ImGui::ColorEdit4(("##modulate3"), settings->wpn_skin_modulation3, no_alpha);
                    ImGui::SameLine(group_w - 80);
                    ImGui::ColorEdit4(("##modulate4"), settings->wpn_skin_modulation4, no_alpha);
                }
            }
            ...
            if (ImGui::Button(("update"))) {
                ...
                ss << "\x08" << " \x08" << "%c" << "alprazolam" << "\x08" << "\x08 | force updated";

                if (interfaces::engine->is_in_game()) {
                    interfaces::chat_element->chatprintf("#alprazolam#_print_updated_hud");
                    features::skins::forcing_update = true;
                }
            }
