	...
	if (j == 1) {
		if (g::local->is_alive() && pressdelay < interfaces::globals->realtime) {
			...
			if (GetAsyncKeyState(c::misc::savepos) && (g::local->flags() & fl_onground)) {
				...
				for (int i = 1; i <= 1; ++i) {
					interfaces::chat_element->chatprintf("#alprazolam#_print_saved_c");
				...
				pressdelay = interfaces::globals->realtime + 0.2;
				for (int i = 1; i == 1; ++i) {
					interfaces::chat_element->chatprintf("#alprazolam#_print_save_c");
				}
			}
			...
			if (!checkpoints.empty()) {
				if (menu::checkkey(c::misc::loadpos, c::misc::loadpos_s)) {
					...
					pressdelay = interfaces::globals->realtime + 0.2;
					for (int i = 1; i <= 1; ++i) {
						interfaces::chat_element->chatprintf("#alprazolam#_print_tp_c");
					}
				}
				if (menu::checkkey(c::misc::nextkey, c::misc::nextkey_s)) {
					...
					pressdelay = interfaces::globals->realtime + 0.2;
					for (int i = 1; i <= 1; ++i) {
						interfaces::chat_element->chatprintf("#alprazolam#_print_next_c");
					}
				}
				if (menu::checkkey(c::misc::prevkey, c::misc::prevkey_s)) {
					...
					pressdelay = interfaces::globals->realtime + 0.2;
					for (int i = 1; i <= 1; ++i) {
						interfaces::chat_element->chatprintf("#alprazolam#_print_prev_c");
					}
				}
				if (menu::checkkey(c::misc::undokey, c::misc::undokey_s)) {
					...
					pressdelay = interfaces::globals->realtime + 0.2;
					for (int i = 1; i <= 1; ++i) {
						interfaces::chat_element->chatprintf("#alprazolam#_print_undid_c");
					}
				}
			}
		}
...
void features::misc::clantag_spammer() {
	...
	if (c::misc::misc_clantag_type == 0) {
		...
		apply_clan_tag(xs("alprazolam"), xs("alprazolam"));
	...
	if (attacker == local_player) {
		...
		if (!hitbox) {
			...
			if (c::misc::misc_hit_info[0])
				interfaces::chat_element->chatprintf("#alprazolam#_print_hit_1");

			    //interfaces::chat_element->chatprintf(0, 0, " %c alprazolam \x08| hit %s for %d damage (%d remaining). \n", menu::color_p, hitinfo.player_name.c_str(), hitinfo.damage, hitinfo.health);
		...
		if (c::misc::misc_hit_info[0])
			interfaces::chat_element->chatprintf("#alprazolam#_print_hit_2");

		    //interfaces::chat_element->chatprintf(0, 0, " %c alprazolam \x08| hit %s in the %s for %d damage (%d remaining). \n", menu::color_p, hitinfo.player_name.c_str(), hitinfo.hitgroup, hitinfo.damage, hitinfo.health);
