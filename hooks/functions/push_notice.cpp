...
void __fastcall sdk::hooks::push_notice::push_notice(int ecx, int edx, const char* text, int str_len, const char* null) {
	auto print_text = [](int m_ecx, int m_edx, std::string str) {
		...
		std::string this_str = std::vformat(
			xs("<font color=\"{}\">alprazolam</font><font color=\"#7d7d7d\"> |</font><font color=\"{}\"> {}</font>"),
	...
	auto print_text2 = [](int m_ecx, int m_edx, std::string str) {
		...
		std::string this_str = std::vformat(
			...
			std::make_format_args(color1, "alprazolam", parsedJumpInfo));
	...
	case FNV1A::HashConst(("#alprazolam#_print_created")):
		return print_text(ecx, edx, xs("created config."));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_injected")):
		return print_text(ecx, edx, xs("successfully injected!"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_saved")):
		return print_text(ecx, edx, xs("saved config!"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_loaded")):
		return print_text(ecx, edx, xs("loaded config!"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_loaded_aimbot")):
		return print_text(ecx, edx, xs("loaded aimbot settings"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_loaded_mvm")):
		return print_text(ecx, edx, xs("loaded movement settings"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_loaded_visuals")):
		return print_text(ecx, edx, xs("loaded visual settings"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_loaded_misc")):
		return print_text(ecx, edx, xs("loaded misc settings"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_loaded_indicators")):
		return print_text(ecx, edx, xs("loaded indicators settings"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_loaded_skins")):
		return print_text(ecx, edx, xs("loaded skins & models"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_failed")):
		return print_text(ecx, edx, xs("please choose a config."));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_refreshed")):
		return print_text(ecx, edx, xs("refreshed config"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_reloaded")):
		return print_text(ecx, edx, xs("reloaded fonts"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_updated_hud")):
		return print_text(ecx, edx, xs("force updated"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_saved_c")):
		return print_text(ecx, edx, xs("saved checkpoint"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_save_c")):
		return print_text(ecx, edx, xs("failed to save position"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_tp_c")):
		return print_text(ecx, edx, xs("teleported to saved checkpoint"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_next_c")):
		return print_text(ecx, edx, xs("next position"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_prev_c")):
		return print_text(ecx, edx, xs("previous position"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_undid_c")):
		return print_text(ecx, edx, xs("undid position"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_jumpbugged")):
		return print_text(ecx, edx, xs("jumpbug"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_edgebugged")):
		return print_text(ecx, edx, xs("edgebug"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_pixelsurfed")):
		return print_text(ecx, edx, xs("pixelsurf"));
		break;
	case FNV1A::HashConst(("#alprazolam#_print_hit_1")):
		...
		break;
	case FNV1A::HashConst(("#alprazolam#_print_hit_2")):
		...
		break;
	case FNV1A::HashConst(("#alprazolam#_spotify_paused")):
		return spotify_text(ecx, edx, xs("now paused"));
		break;
	case FNV1A::HashConst(("#alprazolam#_spotify_advertisement")):
		return spotify_text(ecx, edx, xs("advertisement is now playing"));
		break;
	//case FNV1A::HashConst(("#alprazolam#_spotify_switch")): // do it urself if u really need it
	//	return spotify_text(ecx, edx, std::vformat(xs("now playing: {}"), std::make_format_args(strtitle)));
	//	break;
	case FNV1A::HashConst(("#alprazolam#_jumpstats")):
